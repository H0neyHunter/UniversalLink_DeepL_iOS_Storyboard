...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    //Uygulama Açıkken işleniyor ve ios 13 ve öncesinde çalışıyor.
    ...
    //Uygulama Kapalı ise işleniyor ve ios 13 ve öncesinde çalışıyor. Ama uygulama yüklü değilken tüm versionlarda çalışıyor
    func application(_ application: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey : Any] = [:]) -> Bool {
        if let queryItems = URLComponents(url: url, resolvingAgainstBaseURL: true)?.queryItems {
            ...
            //Uygulama kapalı iken NotificationCenter işe yaramıyor bunun yerine willconnectto kontrolü user defaults
            //NotificationCenter.default.post(Notification(name: Notification.Name("didReceiveNotification")))
